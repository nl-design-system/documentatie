import clsx from 'clsx';
import React from 'react';
import { AnchorHTMLAttributes } from 'react';
import style from './ExternalLink.module.css';

export const ExternalLink = ({ children, href, className }: AnchorHTMLAttributes<HTMLAnchorElement>) => (
  <a href={href} className={clsx(style['external-link'], className)} target="_blank" rel="noreferrer">
    {children}
    <svg width="13.5" height="13.5" aria-hidden="true" viewBox="0 0 24 24" className={style['external-link__icon']}>
      <path d="M21 13v10h-21v-19h12v2h-10v15h17v-8h2zm3-12h-10.988l4.035 4-6.977 7.07 2.828 2.828 6.977-7.07 4.125 4.172v-11z"></path>
    </svg>
  </a>
);
