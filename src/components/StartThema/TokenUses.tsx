// import tokens from '@nl-design-system-unstable/start-design-tokens/dist/tokens.json';
// import tokenList from '@nl-design-system-unstable/start-design-tokens/dist/list.json';
...
export const TokenUses = ({ tokenType }) => {
  ...
  return Object.keys(tokenUses).map((tokenName) => (
    <div key={tokenName}>
      <h4 className="utrecht-heading-4">{tokenName}</h4>
      <ul role="list" className="utrecht-unordered-list">
        {tokenUses[tokenName]?.map((tokenUse, index) => (
          <li key={tokenName + tokenUse + index} className="utrecht-unordered-list__item">
            <code className="utrecht-code">{tokenUse}</code>
          </li>
        ))}
      </ul>
    </div>
