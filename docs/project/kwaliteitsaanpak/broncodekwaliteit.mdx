...
Gebruik waarm mogelijk tools om code automatisch op te maken, zoals Prettier en EditorConfig, voor alle bestandsformaten die vaak worden gebruikt en waar goede tools voor zijn.

Gebruik waar mogelijk UTF-8 als encoding, zoals in tekstbestanden, i18n bestanden etcetera.

## Aanbevolen tools \{#tools\}

Gebruik waar de aanbevolen tools in elke repository waar deze bestandsformaten worden gebruikt:
...
Wanneer je geen tijd hebt om de fouten op te lossen, dan kun je met `git commit --no-verify` de commit doen zonder checks, zodat je in elk geval een backup hebt van je code. [Continuous integration](./continuous-integration.mdx) checks zullen voorkomen dat deze onvoltooide code per ongeluk al wordt gemerged.
