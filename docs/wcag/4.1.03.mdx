...
description: Een statusbericht is een melding die op de pagina verschijnt zonder dat de focus verplaatst. Markeer die in de code als live region, zodat ook screenreaders het bericht voorlezen aan bezoekers die het niet zien.
slug: 4.1.3
keywords:
  - WCAG
  - AA
...
Plaats de live region als lege container al in de HTML bij het laden van de pagina, en vul de tekst later in via JavaScript. Een screenreader registreert een live region pas als die in de DOM staat. Verschijnt het element en de tekst tegelijk, dan mist de screenreader het bericht.

## Veelgemaakte fouten

### Bevestigingsmelding na het verzenden van een formulier wordt niet voorgelezen

Een bezoeker verstuurt een formulier en er verschijnt een bevestiging op de pagina. De pagina wordt niet herladen, de melding verschijnt dynamisch. Zonder `role="status"` op het element negeert de screenreader het bericht.
...
Een bezoeker verstuurt een formulier en de server geeft fouten terug: verplichte velden ontbreken of een datum klopt niet. Bovenaan de pagina verschijnt een samenvatting van de fouten. De pagina wordt niet herladen en de focus blijft op de verzendknop staan. Zonder ARIA-markering of focusverplaatsing leest een screenreader niets voor: de bezoeker denkt dat het verzenden is gelukt.
...
**Wie kan dit oplossen:** een developer plaatst twee lege live regions in het HTML-template (één met `role="status"` voor een geslaagde verificatie en één met `role="alert"` voor een mislukte poging) en vult de juiste container bij het resultaat.
...
**Wie kan dit oplossen:** een developer plaatst een toast-container met `role="status"` in het HTML-template, voegt nieuwe toasts in die container toe en zorgt voor voldoende zichtbaarheidsduur.
...
8. Gebruik <span lang="en">axe DevTools</span> voor een eerste scan. De meeste statusberichtproblemen zijn niet automatisch te herkennen en vereisen handmatig testen met een screenreader.

## Gerelateerde succescriteria

- [3.3.1 Foutidentificatie](/wcag/3.3.1/): foutmeldingen bij formuliervelden die de focus krijgen, vallen onder foutidentificatie. Statusberichten zijn meldingen die verschijnen zonder dat de focus verplaatst.
