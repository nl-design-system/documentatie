...
Op basis van het `type`-attribuut kan een browser soms al raden welk soort gegevens een veld vraagt — bijvoorbeeld bij `type="email"` of `type="tel"`. Voor het accuraat invullen van specifieke persoonlijke gegevens, zoals voornaam, huisnummer of postcode, is een `autocomplete`-attribuut met de juiste waarde nodig. Pas dan weet de browser welke opgeslagen gegevens in welk veld horen. Het `autocomplete`-attribuut maakt het ook mogelijk om iconen of labels bij velden te tonen die de bezoeker helpen begrijpen wat er wordt verwacht — onafhankelijk van de taal van de pagina.
...
Een formulier heeft `autocomplete="off"` op velden die persoonlijke gegevens vragen, of op het `<form>`-element zelf. Een waarde op het `<form>`-element geldt voor alle velden in dat formulier. De browser mag de velden dan niet automatisch invullen. De bezoeker moet alles handmatig typen. Developers doen dit vaak om beveiligingsredenen, maar voor de meeste velden is het niet nodig en het maakt het formulier onnodig moeilijk te gebruiken.
...
- [1.3.1 Info en relaties](/wcag/1.3.1/): labels zijn programmatisch gekoppeld aan hun invoerveld. Autocomplete voegt daar het doel van het veld aan toe.
- [3.3.2 Labels of instructies](/wcag/3.3.2/): zichtbare labels beschrijven wat er in een veld moet worden ingevuld. Autocomplete helpt de browser om het veld automatisch in te vullen op basis van die beschrijving.
- [3.3.7 Overbodige invoer](/wcag/3.3.7/): informatie die de bezoeker eerder heeft ingevuld, wordt automatisch aangeboden. Autocomplete is een van de manieren om dit te realiseren.
