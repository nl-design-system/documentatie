---
title: Form field partial label
hide_title: true
hide_table_of_contents: false
sidebar_label: Form field partial label
pagination_label: Form field partial label
description: Toont een een tekstlabel voor een invoerveld dat deel uitmaakt van de Text input group.
slug: /form-field-partial-label
sidebar_custom_props:
  illustration: FormFieldPartialLabelSketch
---

{/* @license CC0-1.0 */}

import componentProgress from "@nl-design-system/component-progress/dist/index.json";
import { Backlog, DefinitionOfDone, Implementations, Introduction } from "../../../src/components/ComponentPage";

{/* Add name and description for the component */}
export const title = "Form field partial label";
export const description = "Toont een een tekstlabel voor een invoerveld dat deel uitmaakt van de Text input group.";
export const illustration = "FormFieldPartialLabelSketch";
export const issueNumber = 180;

export const component = componentProgress.find((item) => item.number === issueNumber);

<Introduction component={component} headingLevel={1} description={description} illustration={illustration} />

## Definition of Done

<DefinitionOfDone component={component} headingLevel={3} />

## Implementaties

<Implementations component={component} headingLevel={3} />

## Levende Backlog

<Backlog component={component} headingLevel={3} />
