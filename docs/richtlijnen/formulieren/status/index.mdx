---
title: Status in een formulier
title_sm: Status
hide_title: true
hide_table_of_contents: false
sidebar_label: Introductie status
sidebar_position: 12
navigation_order: 12
pagination_label: Status in formulieren
description: Richtlijnen voor het omgaan met status in formulieren.
slug: /richtlijnen/formulieren/status/
keywords:
  - informatie
  - status
  - formulier
  - design
  - code
---

{/* @license CC0-1.0 */}

import FooterInfo from "@nl-design-system-unstable/documentation/richtlijnen/_footer_info.md";

# Richtlijnen NL Design System voor statusberichten in een formulier

Het is belangrijk een gebruiker te informeren over veranderingen en updates.

- Is een formulier succesvol verzonden?
- Zijn er fouten?
- Hoeveel zoekresultaten zijn er?
- Is de tijdslimiet bijna verstreken?

Dit soort veranderingen binnen een pagina kun je met een [statusbericht](https://www.w3.org/Translations/WCAG22-nl/#dfn-status-messages) aangeven.

Een statusbericht moet voor iedereen beschikbaar zijn, en dus ook goed werken met [screenreaders](/woordenlijst/#screenreader) en zoom-instellingen. Ook moet er voldoende tijd zijn om de content in je op te nemen.

## Overzicht richtlijnen

<OverviewPage excludeDocIDs={["richtlijnen/formulieren/status/index"]} />

<FooterInfo />
