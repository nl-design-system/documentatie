...
Wanneer je geen tijd hebt om de fouten op te lossen, maar wel een backup wil committen, dan kun je met `git commit --no-verify` de commit doen zonder checks. [Continuous integration](./continuous-integration.mdx) checks zullen voorkomen dat deze onvoltooide code per ongeluk wordt gemerged.
...
Gebruik `pnpm run --if-present lint` als verplichte check in je continuous integration pipeline, zoals in GitHub Actions om Pull Requests te controleren. Lees meer in het artikel over [continuous integration](/handboek/kernteam/kwaliteitsaanpak/continuous-integration/).
