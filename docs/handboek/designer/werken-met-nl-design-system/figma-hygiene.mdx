...
Hier beschrijven we onze opzet, structuur en afspraken voor Figma, oftewel de zogenoemde 'Figma hygiëne'.

Vooral rondom componenten is het fijn als we dezelfde hygiëne aanhouden. Zo zijn componenten gemakkelijk uitwisselbaar tussen organisaties.
...
Dit is simpelweg een schetsboek dat we gebruiken om snel ideeën uit te werken die gedeeld kunnen worden of later als naslagwerk kunnen dienen.

[Bekijk het 'NL Design System - Schetsbook' in Figma](https://www.figma.com/design/fy08SZpZmqx6ljLwvA3Woe/NL-Design-System---Schetsboek?node-id=231-73&t=f7kKKaXvqOj1q3Ey-1)

##### Voorbeeld bibliotheken

Duplicaten van onze Figma bibliotheken met het [Voorbeeld thema](/handboek/huisstijl/themas/voorbeeld-thema). We gebruiken deze bibliotheken om na te bootsen hoe organisaties met de Figma bibliotheken kunnen werken.
...
Een bestand met complete templates, opgebouwd uit componenten afkomstig uit de 2 Figma bibliotheken met het Voorbeeld thema.
...
Elk Figma bestand heeft als eerste pagina een eigen 'Cover'. Deze cover fungeert als de thumbnail die je instelt en die zichtbaar is in het overzicht van bestanden. Je kunt de kleurstelling en titel aanpassen. Gebruik de functionaliteit ‘Set as thumbnail’ om deze cover te updaten.
...
Standaard zijn Figma bestanden niet openbaar voor iedereen. Aangezien we open source werken, is het belangrijk om bij het delen van linkjes met de community de juiste instellingen te kiezen. Ga bij 'Share' naar de optie 'Anyone can view' om het bestand voor iedereen zichtbaar te maken.
...
Soms zijn componenten opgesplitst in [sub-componenten](#3-3-sub-compenten). Deze sub-componenten mogen een korte naam hebben, zodat de lagen beter leesbaar blijven. We schrijven deze altijd in 'kebab-case' en plaatsen 2 underscores voor de naam. Dit zorgt ervoor dat ze verborgen blijven bij het gebruik van de bibliotheek en sluit aan bij de BEM-conventie, die ook in code wordt gebruikt. Bijvoorbeeld:

- `.denhaag-contact-timeline__step-header` → `__step-header`
- `.denhaag-contact-timeline__step` → `__step`
- `.todo-modal-dialog-header` → `__header`
...
#### 3.5.2 Volgorde van property values
...
Feedback uit Figma structuur Google Form.
...
Voor frames hanteren we de volgende structuur voor de schrijfwijze: `Viewport_Naam-van-pagina_Aanvulling--state`. `SV` staat voor 'small viewport', `LV` staat voor 'large viewport'.
