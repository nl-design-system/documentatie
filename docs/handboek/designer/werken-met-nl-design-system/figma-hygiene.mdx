...
Sommige componenten bestaan uit [sub-componenten](#3-3-sub-componenten). Geef deze een korte naam. Zo blijven de lagen leesbaar. Schrijf sub-componenten in 'kebab-case'. Gebruik twee underscores voor de naam. Laat de naam van de hoofdcomponent weg. Zo blijven ze verborgen in de bibliotheek. Ook sluit de naamgeving aan bij de BEM-methode uit de code.

Bijvoorbeeld:

- `__step-header` (in plaats van `.denhaag-contact-timeline__step-header`)
- `__step` (in plaats van `.denhaag-contact-timeline__step`)
- `__button` (in plaats van `.utrecht-accordion__button`)
...
Ontbreekt de component die je nodig hebt? Ontwikkel deze dan volgens de NL Design System-architectuur.
