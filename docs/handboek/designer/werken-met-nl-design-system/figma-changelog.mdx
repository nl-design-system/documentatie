...
In de Figma bibliotheek zitten enkele handige 'Helper' componenten. Tot nu toe waren de meeste verborgen, doordat er een ‘.’ vooraan de naam stond. Dit zorgde er echter ook voor dat deze componenten niet naar voren kwamen bij het gebruik van de 'Swap Library' functionaliteit. Een stap die elke keer gezet moet worden wanneer je een nieuw component wilt overnemen uit de NL Design System - Bibliotheek. Dus we hebben ze niet langer verborgen én de naamgeving van deze componenten aangepast.
...
- Voer dezelfde naamgeving door bij de Helper componenten in de bibliotheek van jouw organisatie. Je herkent ze aan de prefix 'Helper\_'.
- Publiseer de bibliotheek.
- Accepteer de update in de ToDo Bibliotheek van jouw organisatie.

:::info[Goed om te weten]

Het viel ons op dat de meeste Helper componenten in de ToDo Bibliotheek keurig de nieuwe naam kregen. Alleen '.Page Heading' niet. Deze zul je daarom handmatig via de 'Swap Instance' functionaliteit moeten inwisselen voor 'Helper_Page-Heading'. Door in de ToDo Bibliotheek te zoeken op de term '.Page Heading' kun je ze snel nalopen.
