...
NL Design System wil de beste componenten uit de community herbruikbaar maken voor de hele overheid. Daarom hebben de componenten van NL Design System van zichzelf geen huisstijl. Iedere organisatie kan zijn eigen huisstijl op de componenten toepassen. Om dat voor elkaar te krijgen maken we gebruik van ‘design tokens’.
...
![De kleur #FA5757 verwijst naar 'example.color.red.500', het lettertype Oswald verwijst naar 'example.typography.primary.font-family'.](https://raw.githubusercontent.com/nl-design-system/documentatie/assets/meedoen_design-tokens_example.png)
...
Deze data beheer je vanuit een platform onafhankelijk bestand. Bij NL Design System hebben we gekozen voor JSON. We gebruiken [Style Dictionary](https://styledictionary.com/) om het JSON bestand te vertalen. Hierdoor zijn de design tokens door diverse technieken en tools te gebruiken. Denk aan CSS, React, Angular, maar ook Figma.
...
Bij NL Design System verdelen we design tokens over drie niveaus.
...
![De kleur #1B59A4 verwijst naar 'example.color.donkerblauw' met het label 'Brand'.](https://raw.githubusercontent.com/nl-design-system/documentatie/assets/img_design-tokens_brand.png)
...
![De kleur #1B59A4 verwijst naar 'example.color.donkerblauw' met het label 'Brand'. 'example.color.donkerblauw' verwijst naar 'basis.color.action-1-inverse.bg-default' met het label 'Common'.](https://raw.githubusercontent.com/nl-design-system/documentatie/assets/img_design-tokens_common.png)

:::tip[Tip]

Vanuit NL Design System bieden we organisaties een 'basis-set' van Common tokens aan genaamd 'Basis-tokens'. Door organisatieoverstijgend uit dezelfde bron te werken, worden componenten eenvoudig uitwisselbaar tussen organisaties.

[Bekijk de basis-tokens documentatie](/handboek/huisstijl/basis-tokens)

:::

### Component tokens

Dit zijn alle tokens die nodig zijn voor een component. Ze verkrijgen hun waarde vanuit het Common niveau. Denk aan de achtergrondkleur van een button of de randkleur van een invulveld bij een foutmelding.

![De kleur #1B59A4 verwijst naar 'example.color.donkerblauw' met het label 'Brand'. 'example.color.donkerblauw' verwijst naar 'basis.color.action-1-inverse.bg-default' met het label 'Common'. 'basis.color.action-1-inverse.bg-default' verwijst naar 'nl.button.primary.background-color' met het label 'Component'.](https://raw.githubusercontent.com/nl-design-system/documentatie/assets/img_design-tokens_component.png)

[Bekijk het componenten overzicht](/componenten/)

### Wat is het voordeel van drie niveaus?

- Herbruikbare componenten: componenten zijn niet rechtstreeks afhankelijk van een specifieke huisstijl en kunnen daardoor met verschillende thema’s worden gebruikt.
- Makkelijk te thematiseren: component tokens verwijzen standaard naar Common tokens. Daardoor kun je een huisstijl centraal instellen, zonder ieder component afzonderlijk te configureren.
- Gericht uitzonderingen maken: waar nodig kan er op component-niveau een uitzondering worden gemaakt. Zo kun je specifieke huisstijlkeuzes of de styling van geneste componenten bepalen zonder de Common tokens aan te passen.

Zo combineer je centrale consistentie met ruimte voor specifieke toepassingen. Kortom: ieder niveau heeft een eigen verantwoordelijkheid. Brand bepaalt de beschikbare waarden, Common geeft ze een betekenis en Component bepaalt hoe ze worden toegepast.
...
Naam van de organisatie, bijvoorbeeld `utrecht` of `denhaag`. De prefix `nl` wordt alleen gebruikt door het kernteam van NL Design System. Elk team kiest een eigen prefix, met alleen de letters `a` tot `z` en zonder koppelstreepje.
...
<VideoPlayer id="jSvjnU7oY5Y" title="Design tokens vastleggen" />

### Design tokens inzetten

<VideoPlayer id="-guAi7Lvl6g" title="Design tokens inzetten" />

## Meer informatie

### Links

[Style Dictionary](https://styledictionary.com/)<br/>
