...
We gebruiken OpenID Connect (OIDC) om packages te publiceren.
...
Goed om te weten: publiceren met OpenID Connect kan op het moment alleen met GitHub, GitLab en CircleCI.

### Nieuwe packages publiceren \{#nieuwe-packages-publiceren\}
...
3. Deel de links naar de gepubliceerde npm packages met het kernteam. Het kernteam zal dan [trusted publishing](https://docs.npmjs.com/trusted-publishers) instellen via npmjs.org. Nieuwe versies hebben daarna geen access token meer nodig.
...
Zie [Nieuwe packages publiceren](#nieuwe-packages-publiceren).
