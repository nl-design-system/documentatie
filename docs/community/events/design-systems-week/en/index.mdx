...
{!!settings.isUpcomingOrHappening && (<>

<Paragraph lead>
  NL Design System is organising the Design Systems Week for the {settings.en.edition} time. It will feature a number of short talks
...
<ActionGroup>
  <ButtonLink href={`/events/design-systems-week-${settings.year}/en/sign-up`} appearance="primary-action-button">
    {"Sign up"}
    <IconChevronRight />
  </ButtonLink>
...
{!!settings.isUpcomingOrHappening && (`This is the ${settings.en.edition} year that NL Design System is organizing Design Systems Week. Can't wait? Then check out the videos from previous years!`)}

{!settings.isUpcomingOrHappening && ("Check out the videos from previous years.")}
