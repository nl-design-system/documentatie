import type { SidebarsConfig } from '@docusaurus/plugin-content-docs';

const sidebars: SidebarsConfig = {
  introductie: [
    {
      type: 'category',
      label: 'Introductie',
      collapsible: false,
      className: 'sidebar__main-category',
      link: {
        type: 'doc',
        id: 'introductie/index',
      },
      items: [{ type: 'autogenerated', dirName: 'introductie' }],
    },
  ],
  handboek: [
    {
      type: 'category',
      label: 'Handboek',
      collapsible: false,
      className: 'sidebar__main-category',
      link: {
        type: 'generated-index',
        title: 'Handboek',
        slug: 'handboek',
        keywords: ['Handboek', 'Meedoen', 'overzicht'],
      },
      items: [
        { type: 'doc', id: 'handboek/componenten-vinden' },
        {
          type: 'category',
          label: 'Definition of Done',
          description:
            'Hier vind je waar we op letten en hoe je met een stappenplan aan de Definition of Done voor elke status in het Estafettemodel kan voldoen.',
          link: {
            type: 'doc',
            id: 'handboek/bijdragen/index',
          },
          items: [{ type: 'autogenerated', dirName: 'handboek/bijdragen' }],
        },
        {
          type: 'category',
          label: 'Huisstijl vastleggen',
          description: 'Je huisstijl vastleggen als thema bij NL Design System? Hier vind je hoe je dat aanpakt.',
          link: {
            type: 'generated-index',
            title: 'Huisstijl vastleggen',
            slug: '/handboek/huisstijl-vastleggen/overzicht',
            keywords: ['huisstijl', 'design tokens', 'overzicht'],
          },
          items: [{ type: 'autogenerated', dirName: 'handboek/huisstijl-vastleggen' }],
        },
        {
          type: 'category',
          label: 'Voor designers',
          description:
            'Mee designen aan NL Design System of gebruik maken van bestaande Figma componenten? Als designer vind je hier alle documentatie die je nodig hebt.',
          link: {
            type: 'doc',
            id: 'handboek/designer/index',
          },
          items: [{ type: 'autogenerated', dirName: 'handboek/designer' }],
        },
        {
          type: 'category',
          label: 'Voor developers',
          description:
            'Mee ontwikkelen aan NL Design System of gebruik maken van bestaande componenten? Als developer vind je hier alle documentatie die je nodig hebt om te beginnen.',
          link: {
            type: 'doc',
            id: 'handboek/developer/index',
          },
          items: [{ type: 'autogenerated', dirName: 'handboek/developer' }],
        },
        {
          type: 'category',
          label: 'Voor organisaties',
          link: {
            type: 'generated-index',
            title: 'Voor organisaties',
            description:
              'Overweeg je NL Design System te gebruiken? Hier verzamelen we alle informatie die je als organisatie mogelijk kan helpen.',

            slug: '/handboek/organisatie/overzicht',
            keywords: ['organisaties', 'overzicht'],
          },
          items: [{ type: 'autogenerated', dirName: 'handboek/organisatie' }],
        },
        {
          type: 'category',
          label: 'Voor het kernteam',
          description:
            'Werk je in opdracht van NL Design System? Hier vind je de kwaliteitsaanpak en de schrijfwijzer die het kernteam gebruikt.',
          link: {
            type: 'doc',
            id: 'handboek/kernteam/index',
          },
          items: [
            {
              type: 'category',
              label: 'Kwaliteitsaanpak',
              link: {
                type: 'doc',
                id: 'handboek/kernteam/kwaliteitsaanpak/index',
              },
              items: [{ type: 'autogenerated', dirName: 'handboek/kernteam/kwaliteitsaanpak' }],
            },
            {
              type: 'category',
              label: 'Schrijfwijzer',
              link: {
                type: 'doc',
                id: 'handboek/kernteam/schrijfwijzer/index',
              },
              items: [{ type: 'autogenerated', dirName: 'handboek/kernteam/schrijfwijzer' }],
            },
          ],
        },
        {
          type: 'category',
          label: 'Baseline',
          link: {
            type: 'doc',
            id: 'baseline/index',
          },
          items: [{ type: 'autogenerated', dirName: 'baseline' }],
        },
        { type: 'doc', id: 'woordenlijst/index' },
      ],
    },
  ],
  richtlijnen: [
    {
      type: 'category',
      label: 'Richtlijnen',
      description:
        'Onze richtlijnen om te helpen met het ontwikkelen van begrijpelijke, gebruiksvriendelijke én toegankelijke online dienstverlening. Onze richtlijnen zijn zoveel mogelijk onderbouwd met gebruikersonderzoek.',
      collapsible: false,
      className: 'sidebar__main-category',
      link: {
        type: 'doc',
        id: 'richtlijnen/index',
      },
      items: [
        {
          type: 'doc',
          id: 'richtlijnen/index',
        },
        {
          type: 'category',
          label: 'Stijl',
          description: 'Onze richtlijnen voor een toegankelijke huisstijl en visueel ontwerp.',
          link: {
            type: 'doc',
            id: 'richtlijnen/stijl/index',
          },
          items: [{ type: 'autogenerated', dirName: 'richtlijnen/stijl' }],
        },
        {
          type: 'category',
          label: 'Content',
          link: {
            type: 'doc',
            id: 'richtlijnen/content/index',
          },
          items: [{ type: 'autogenerated', dirName: 'richtlijnen/content' }],
        },
        /*{
          type: 'category',
          label: 'Code',
          link: {
            type: 'doc',
            id: 'richtlijnen/code/index',
          },
          items: [{ type: 'autogenerated', dirName: 'richtlijnen/code' }],
        },*/
        {
          type: 'category',
          label: 'Formulieren',
          description: 'Onze richtlijnen voor inclusieve formulieren.',
          link: {
            type: 'doc',
            id: 'richtlijnen/formulieren/index',
          },
          items: [{ type: 'autogenerated', dirName: 'richtlijnen/formulieren' }],
        },
        {
          type: 'category',
          label: 'WCAG',
          description:
            "Hier proberen WCAG begrijpelijk uit te leggen en daardoor makkelijker toepasbaar te maken. Daarnaast staat beschreven hoe je webpagina's voor de verschillende succescriteria kunt testen.",
          link: {
            type: 'doc',
            id: 'wcag/index',
          },
          items: [{ type: 'autogenerated', dirName: 'wcag' }],
        },
        {
          type: 'category',
          label: 'Changelog',
          link: {
            type: 'doc',
            id: 'CHANGELOG',
          },
          items: [],
        },
      ],
    },
  ],
  componenten: [
    {
      type: 'category',
      label: 'Componenten',
      collapsible: false,
      className: 'sidebar__main-category',
      items: [{ type: 'autogenerated', dirName: 'componenten' }],
    },
  ],
  voorbeelden: [
    {
      type: 'category',
      label: 'Voorbeelden',
      collapsible: false,
      className: 'sidebar__main-category',
      description:
        "Hier tonen we voorbeelden van veelvoorkomende pagina's en klantreizen, gemaakt met richtlijnen en componenten uit NL Design System.",
      link: {
        type: 'doc',
        id: 'voorbeelden/index',
      },
      items: [
        { type: 'doc', id: 'voorbeelden/index' },
        { type: 'doc', id: 'voorbeelden/gebruikersonderzoek' },
        {
          type: 'category',
          label: 'Patronen en templates',
          link: {
            type: 'doc',
            id: 'voorbeelden/patronen-en-templates/index',
          },
          items: [{ type: 'autogenerated', dirName: 'voorbeelden/patronen-en-templates' }],
        },
      ],
    },
  ],
  community: [
    {
      type: 'category',
      label: 'Community',
      collapsible: false,
      className: 'sidebar__main-category',
      link: {
        type: 'generated-index',
        title: 'Community',
        slug: 'community',
        keywords: ['Community', 'overzicht'],
      },
      items: [
        { type: 'doc', id: 'community/sluit-je-aan' },
        { type: 'doc', id: 'community/wie-doet-mee' },
        {
          type: 'doc',
          id: 'community/community-sprints/index',
        },
        {
          type: 'category',
          label: 'MijnServices Community',
          description:
            'De MijnServices Community Sprint maakt deel uit van een samenwerkingsverband om NL Design System-componenten te ontwikkelen voor projecten die moeten voldoen aan de MijnServices patronen.',
          link: {
            type: 'doc',
            id: 'community/community-sprints/mijn-services-community/index',
          },
          items: [
            {
              type: 'doc',
              id: 'community/community-sprints/mijn-services-community/index',
            },
            { type: 'doc', id: 'community/community-sprints/mijn-services-community/aanmelden' },
          ],
        },
        {
          type: 'category',
          label: 'Rijkshuisstijl Community',
          description:
            'De Rijkshuisstijl Community Sprint maakt deel uit van een samenwerkingsverband om NL Design System-componenten te ontwikkelen voor projecten die moeten voldoen aan de Rijkshuisstijl.',
          link: {
            type: 'doc',
            id: 'community/community-sprints/rijkshuisstijl-community/index',
          },
          items: [
            {
              type: 'doc',
              id: 'community/community-sprints/rijkshuisstijl-community/index',
            },
            { type: 'doc', id: 'community/community-sprints/rijkshuisstijl-community/aanmelden' },
          ],
        },
        {
          type: 'category',
          label: 'Expertteam Digitale Toegankelijkheid',
          link: {
            type: 'doc',
            id: 'community/expertteam-digitale-toegankelijkheid/index',
          },
          items: [
            {
              type: 'doc',
              id: 'community/expertteam-digitale-toegankelijkheid/design-tokens-lint/index',
            },
          ],
        },
        {
          type: 'category',
          label: 'Bijeenkomsten',
          description:
            'NL Design System organiseert regelmatig bijeenkomsten, online en offline, met als doel om samen te werken en kennis te delen.',
          collapsible: true,
          link: {
            type: 'generated-index',
            title: 'Bijeenkomsten',
            slug: '/community/events/overzicht',
          },
          items: [
            {
              type: 'category',
              label: 'Heartbeat',
              link: {
                type: 'generated-index',
                title: 'Heartbeat',
                slug: '/events/heartbeat',
              },
              description:
                "In onze tweewekelijkse Heartbeat vertellen kernteam en community wat er gaande is. Meld je aan of kijk video's terug.",
              items: [
                { type: 'doc', id: 'community/events/heartbeat/index' },
                { type: 'doc', id: 'community/events/heartbeat/videos' },
                { type: 'doc', id: 'community/events/heartbeat/aanmelden' },
                { type: 'doc', id: 'community/events/heartbeat/zelf-iets-vertellen' },
              ],
            },
            {
              type: 'category',
              label: 'Design Open Hour',
              description:
                'Tijdens de (online) Design Open Hour delen designers van verschillende organisaties kennis.',
              link: {
                type: 'generated-index',
                title: 'Design Open Hour',
                slug: '/events/design-open-hour',
              },
              items: [
                { type: 'doc', id: 'community/events/design-open-hour/index' },
                { type: 'doc', id: 'community/events/design-open-hour/aanmelden' },
              ],
            },
            {
              type: 'category',
              label: 'Developer Open Hour',
              description:
                'Tijdens de (online) Developer Open Hour delen developers van verschillende organisaties kennis.',
              link: {
                type: 'generated-index',
                title: 'Developer Open Hour',
                slug: '/events/developer-open-hour',
              },
              items: [
                { type: 'doc', id: 'community/events/developer-open-hour/index' },
                { type: 'doc', id: 'community/events/developer-open-hour/aanmelden' },
              ],
            },
            {
              type: 'doc',
              id: 'community/events/po-open-hour',
            },
            { type: 'doc', id: 'community/events/mijn-services-check-in' },
            { type: 'doc', id: 'community/events/rijkshuisstijl-community-open-hour' },
            { type: 'doc', id: 'community/events/design-open-dag' },
            { type: 'doc', id: 'community/events/estafettemodeldag' },
          ],
        },
        {
          type: 'category',
          label: 'Design Systems Week',
          link: {
            type: 'doc',
            id: 'community/events/design-systems-week/index',
          },
          items: [
            { type: 'doc', id: 'community/events/design-systems-week/index' },
            { type: 'doc', id: 'community/events/design-systems-week/programma' },
            { type: 'doc', id: 'community/events/design-systems-week/tijdschema' },
            {
              type: 'category',
              label: 'English',
              link: { type: 'doc', id: 'community/events/design-systems-week/en/index' },
              items: [
                { type: 'doc', id: 'community/events/design-systems-week/en/index' },
                { type: 'doc', id: 'community/events/design-systems-week/en/program' },
                { type: 'doc', id: 'community/events/design-systems-week/en/timetable' },
                {
                  type: 'category',
                  label: 'Previous editions',
                  link: { type: 'doc', id: 'community/events/design-systems-week/en/previous-editions/2025' },
                  items: [
                    { type: 'autogenerated', dirName: 'community/events/design-systems-week/en/previous-editions' },
                  ],
                },
              ],
            },
            {
              type: 'category',
              label: 'Eerdere edities',
              link: { type: 'doc', id: 'community/events/design-systems-week/eerdere-edities/2025' },
              items: [{ type: 'autogenerated', dirName: 'community/events/design-systems-week/eerdere-edities' }],
            },
          ],
        },
        { type: 'doc', id: 'community/design-systems' },
        { type: 'doc', id: 'community/global-design-system' },
        { type: 'doc', id: 'community/belangenorganisaties/aanmelden' },
        { type: 'doc', id: 'community/slack' },
      ],
    },
  ],
  project: [
    {
      type: 'category',
      label: 'Project',
      collapsible: false,
      className: 'sidebar__main-category',
      link: {
        type: 'doc',
        id: 'project/index',
      },
      items: [
        { type: 'doc', id: 'project/faq' },
        { type: 'doc', id: 'project/kernteam' },
        { type: 'doc', id: 'project/blijf-op-de-hoogte' },
        { type: 'doc', id: 'project/klachten' },
      ],
    },
  ],
};

export default sidebars;
